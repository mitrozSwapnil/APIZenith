...
namespace ZenithApp.ZenithRepository
{
    public class CustomerRepository : BaseRepository
    {
        ...
        private readonly IMongoCollection<tbl_Master_Category> _mastercategory;
        ...
        public CustomerRepository(IOptions<MongoDbSettings> settings, IHttpContextAccessor acc, IConfiguration configuration, S3Repository s3Repository)
        {
            ...
            _mastercategory = database.GetCollection<tbl_Master_Category>("tbl_Master_Category");
        ...
        public async Task<addCustomerApplicationResponse> AddCustomerApplication(addCustomerApplicationRequest request)
        {
            var response = new addCustomerApplicationResponse();
            // 🔹 Step 1: Get logged-in user info
            var UserId = _acc.HttpContext?.Session.GetString("UserId");

            // If session missing, handle early
            if (string.IsNullOrEmpty(UserId))
            {
            ...
            try
            {
                ...
                // 🔹 Step 4: Determine Final Submit & Status
                bool? isFinalSubmit = null;
                DateTime? submitDate = null;
                string status = "687a2925694d00158c9bf269"; // Default status (In Progress)
                ...
                // 🔹 Step 5: Find existing application
                var existing = _customer.Find(x => x.Id == request.ApplicationId).FirstOrDefault();

                if (existing != null)
                {
                    var update = Builders<tbl_customer_application>.Update
                        .Set(x => x.ApplicationName, request.ApplicationName)
                        ...
                        .Set(x => x.Expected_Audit_Date, request.Expected_Audit_Date)
                        .Set(x => x.Holiday, request.Holiday)
                        .Set(x => x.Audit_Language, request.Audit_Language)
                        .Set(x => x.ConsultantName, request.ConsultantName)
                        .Set(x => x.FileName, request.FileName)
                        .Set(x => x.Name, request.Name)
                        .Set(x => x.Contact_details, request.Contact_details)
                        .Set(x => x.Datetime, request.Datetime)
                        .Set(x => x.status, status)
                        .Set(x => x.Designation, request.Designation)
                        // .Set(x => x.Fk_Annaxture, request.Fk_Annaxture)
                        .Set(x => x.Fk_ApplicationCertificates, request.Fk_ApplicationCertificates?.Select(cert => new ApplicationCertificate
                        {
                            Id = string.IsNullOrEmpty(cert.Id)? ObjectId.GenerateNewId().ToString(): cert.Id,
                            Certificate_Name = cert.Certificate_Name,
                            Is_Delete = cert.Is_Delete,
                            

                            // ✅ Properly map nested subtype list
                            SubType = cert.SubType?.Select(st => new SubTypeItem
                            {
                                Id = string.IsNullOrEmpty(st.id)
                                    ? ObjectId.GenerateNewId().ToString()
                                    : st.id,
                                Scheme = st.Scheme,
                                Annexure = st.Annexure,
                                Url = st.Url
                            }).ToList()
                        }).ToList())

                        .Set(x => x.Fk_Product_Certificates, request.Fk_Product_Certificates)
                        .Set(x => x.Fk_Key_Personnels, request.Fk_Key_Personnels?.Select(k => new KeyPersonnel
                        {
                        ...
                        .Set(x => x.Fk_Customer_Sites, request.Fk_Customer_Sites?.Select(s => new CustomerSite
                        {
                            Address = s.Address,
                        ...
                        .Set(x => x.Fk_Customer_Entity, request.Fk_Customer_Entity?.Select(e => new CustomerEntity
                        {
                            Name_of_Entity = e.Name_of_Entity,
                            Identification_Number = e.Identification_Number,
                            File = e.file
                        }).ToList())
                        .Set(x => x.OutsourceProcess, request.OutsourceProcess)
                        .Set(x => x.IsFinalSubmit, isFinalSubmit)
                        .Set(x => x.SubmitDate, submitDate)
                        .Set(x => x.UpdatedAt, DateTime.Now)
                        .Set(x => x.UpdatedBy, UserId);

                    _customer.UpdateOne(x => x.Id == existing.Id, update);

                    response.Message = "Application updated successfully.";
                }
                else
                {
                    var entity = new tbl_customer_application
                    {
                       ...
                       // Fk_Annaxture = request.Fk_Annaxture,
                        Fk_ApplicationCertificates = request.Fk_ApplicationCertificates?.Select(cert => new ApplicationCertificate
                        {
                        ...
                        Fk_Product_Certificates = request.Fk_Product_Certificates,
                        Fk_Key_Personnels = request.Fk_Key_Personnels?.Select(k => new KeyPersonnel
                        {
                        ...
                        Fk_Customer_Sites = request.Fk_Customer_Sites?.Select(s => new CustomerSite
                        {
                        ...
                        Fk_Customer_Entity = request.Fk_Customer_Entity?.Select(e => new CustomerEntity
                        {
                        ...
                        OutsourceProcess = request.OutsourceProcess,
                        ActiveState = request.ActiveStep ?? 1,
        ...
        //                       .Set(x => x.Holiday,
        //                                 request.Holiday != null && request.Holiday.Any()
        //                                     ? request.Holiday
        //                                     : existingApplication.Holiday)
        ...
        //                        //.Set(x => x.Fk_Annaxture, string.IsNullOrWhiteSpace(result) ? existingApplication.Fk_Annaxture : result)
        //                        .Set(x => x.Fk_Annaxture, existingApplication.Fk_Annaxture)
        ...
        //                                // ✅ Start with existing annexure
        //                                List<AnnexureList> annexureUrl =request.Fk_Annaxture ??  existingCert?.File;
        ...
        //                        // 1. Get all existing certs of this application (type = Product)
        //                        var existingCerts = _customercertificates.Find(x =>
        //                            x.Fk_Customer_Application == existingApplication.Id &&
        ...
        //                                // ✅ Start with existing annexure
        //                                //List<AnnexureList> annexureUrl = existingCert?.File;

        //                                List<AnnexureList> annexureUrl = request.Fk_Annaxture?? existingCert?.File; // Replace only if new file uploaded


        //                                if (existingCert != null)
        //                                {
        //                                    // ✅ Update existing Product cert
        //                                    var updateCert = Builders<tbl_customer_certificates>.Update
        //                                        .Set(x => x.UpdatedAt, DateTime.Now)
        //                                        .Set(x => x.UpdatedBy, UserId)
        //                                        .Set(x => x.status, status)
        ...
        //                        {
        //                            if (string.IsNullOrEmpty(personnel.customerKeyPersonnelId))
        //                            {
        ...
        //                        {
        //                            if (string.IsNullOrEmpty(site.customer_SiteId))
        //                            {
        ...
        //                            if (string.IsNullOrEmpty(entity.customer_EntityId)) // NEW
        //                            {
        ...
        //                                    file = entity.file ?? null, // null if no file uploaded
        //                                    CreatedAt = DateTime.Now,
        //                                    UpdatedAt = DateTime.Now,
        //                                    CreatedBy = UserId,
        //                                    UpdatedBy = UserId,
        ...
        //                                        .Set(x => x.file, entity.file ?? existingEntity.file) // use new file if uploaded
        //                                        .Set(x => x.UpdatedAt, DateTime.Now)
        //                                        .Set(x => x.UpdatedBy, UserId);
        ...
        //                    {
        //                        if (entityList.file != null) // upload only if new file provided
        //                        {
        //                            fileUrl = entityList.file;
        //                        }
        ...
        //                //    .Where(x => x != null)
        //                //    .ToList(),
        //                ApplicationCertificates = _mastercertificate.Find(x => x.Is_Delete == false && x.CertificationType == "Regular").ToList(),
        //                ProductCertificates = _mastercertificate.Find(x => x.Is_Delete == false && x.CertificationType == "Product").ToList(),
        ...
        [HttpGet("GetCustomerApplication")]
        public async Task<getCustomerApplicationResponse> GetCustomerApplication(getCustomerApplicationRequest request)
        {
            var response = new getCustomerApplicationResponse();
            ...
            try
            {
                ...
                if (userType?.Trim().ToLower() != "customer")
                {
                    response.Message = "Invalid token or role.";
                    response.Success = false;
                    response.HttpStatusCode = System.Net.HttpStatusCode.Forbidden;
                    response.ResponseCode = 1;
                    return response;
                ...
                // 🔹 Step 3: Fetch directly from main collection
                var app = _customer
                ...
                if (app == null)
                {
                    response.Message = "No application found for the given ApplicationId.";
                    response.HttpStatusCode = System.Net.HttpStatusCode.NotFound;
                    response.Success = false;
                    response.ResponseCode = 1;
                    return response;
                ...
                // 🔹 Step 4: Exclude internal fields if needed
                app.CreatedBy = null;
                app.UpdatedBy = null;
                app.CreatedAt = default;
                app.UpdatedAt = default;
                // (Optional) — keep if frontend doesn’t need them

                // 🔹 Step 5: Wrap into response
                response.Data = new List<tbl_customer_application> { app };
        ...
        public async Task<userDropdownResponse> GetAllDropdown(userDropdownRequest request)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrWhiteSpace(request.type))
                {
                    ...
                    {
                        case "userlist":
                            response.Data = await _user
                                .Find(x => x.IsDelete == 0 &&
                                           x.Fk_RoleID == "686fc53af41f7edee9b89cd6" &&
                                           x.Type == "Reviewer" &&
                                           x.Department.Contains(request.CertificationName))
                        ...
                        case "regular":
                        case "product":
                            var certs = await _mastercertificate
                                .Find(x => x.Is_Delete == false &&
                                           x.CertificationType != null &&
                                           x.CertificationType.ToLower() == type)
                                .ToListAsync();

                            response.Data = certs.Select(x => new UserDropdownDto
                            {
                                Id = x.Id,
                                Name = x.Certificate_Name,
                                SubTypes = x.SubType != null
                                    ? x.SubType.Select(st => new SubTypeDto
                                    {
                                        Id = st._id,
                                        Scheme = st.Scheme,
                                        Annexure = st.Annexure
                        ...
                        case "techarea":
                            response.Data = await _technicalarea
                        ...
                        case "audittype":
                            response.Data = await _masterAudit
                                .Find(x => x.isDelete == false)
                                .Project(x => new UserDropdownDto
                                {
                                    Id = x.Id,
                        ...
                        case "designation":
                            response.Data = await _masterdesignation
                        ...
                        case "terms":
                            response.Data = await _masterterms
                                .Find(x => x.isDelete == false)
                                .Project(x => new UserDropdownDto
                                {
                                    Id = x.Id,
                        ...
                        case "category":
                        case "subcategory":
                            response.Data = await _mastercategory
                                .Find(x => x.IsDelete == false && x.Type == type)
                                .Project(x => new UserDropdownDto
                                {
                                    Id = x.Id,
        ...
        //                    .Find(x => x.IsDelete == 0 && x.Fk_RoleID== "686fc53af41f7edee9b89cd6" && x.Type== "Reviewer" && x.Department.Contains(request.CertificationName))
        //                    .Project(x => new UserDropdownDto
        //                    {
        //                        Id = x.Id,
        //                        Name = x.FullName
        ...
        //            {
        //                var userList = await _user
        //                     .Find(x => x.IsDelete == 0 && x.Type == "Trainee" && x.Fk_RoleID == "689b444eb211f12a6a722738" && x.Department.Contains(request.CertificationName))
        //                     .Project(x => new UserDropdownDto
        //                     {
        //                         Id = x.Id,
        //                         Name = x.FullName
        ...
        //                    .Find(x => x.Is_Delete == false &&
        //                               x.CertificationType != null &&
        //                               x.CertificationType.ToLower() == "regular")
        //                    .ToListAsync(); // ✅ Use async version if available

        //                response.certificate = products;
        //            }
        ...
        //                    .Find(x => x.Is_Delete == false &&
        //                               x.CertificationType != null &&
        //                               x.CertificationType.ToLower() == "product")
        //                    .ToListAsync(); // ✅ Use async version if available
        ...
        //                response.Data = products;
        //            }
        //            else if (request.type.Trim().ToLower() == "category")
        //            {
        //                var products = await _mastercategory
        //                    .Find(x => x.IsDelete == false && x.Type== "category")
        //                    .Project(x => new UserDropdownDto
        //                    {
        //                        Id = x.Id,
        //                        Name = x.Category_Name
        ...
        //                response.Data = products;
        //            }
        //            else if (request.type.Trim().ToLower() == "subcategory")
        //            {
        //                var products = await _mastercategory
        //                    .Find(x => x.IsDelete == false && x.Type == "subcategory")
        //                    .Project(x => new UserDropdownDto
        //                    {
        //                        Id = x.Id,
        //                        Name = x.Category_Name
        ...
        //                response.Data = products;
        //            }
        //            else if (request.type.Trim().ToLower() == "useraudit")
        //            {
        //                var userList = await _user
        //                    .Find(x => x.IsDelete == 0 &&
        //                                (x.Fk_RoleID == "68f0f445c61aaa8e75ac2448" ||
